    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (isFirstMessage) {
          ...
          systemPrompt = `あなたは${kannonData.name}です。${kannonData.persona}\n\n【まとめと提案の指示】\n1. これまでの対話で問題が明確になってきました。\n2. まず、ユーザーの悩みを簡潔にまとめてください（2-3文程度）。\n3. 次に、具体的な対策や行動の提案を2-3個してください。\n4. 提案は実行可能で、ユーザーが今日から始められるような具体的なものにしてください。\n5. 例: 「まずは、毎日5分だけでも自分のための時間を作ってみてはいかがでしょうか」\n6. 最後に、相談内容に応じて、専門機関や相談窓口を提案してください。\n   - 例: 心の健康に関する悩み→「より専門的なサポートが必要な場合は、心の健康相談統一ダイヤル（0570-064-556）や、かかりつけの医療機関に相談されることをお勧めします」\n   - 例: DVやハラスメント→「より安全な環境を求める場合は、DV相談ナビ（0570-0-55210）や、お住まいの地域の女性相談窓口に相談されることをお勧めします」\n   - 例: 法律問題→「法律的なサポートが必要な場合は、法テラス（0570-078374）や、弁護士会の無料相談をご利用ください」\n   - 例: 働くことに関する悩み→「労働問題については、労働条件相談ホットライン（0120-811-610）や、お住まいの地域の労働基準監督署に相談されることをお勧めします」\n   - 例: 金銭問題→「借金や多重債務については、金融庁の多重債務相談窓口（0570-031640）や、法テラスをご利用ください」\n7. 提案の後、ユーザーがどう感じているか、実行できそうかを確認してください。`;
        } else {
